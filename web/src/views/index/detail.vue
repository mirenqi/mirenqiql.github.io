<template>
  <div class="detail">
    ...
    <div class="detail-content">
      ...
      <div class="main-tab">
        <div class="order-view">
          <span 
            ...
            :key="index"
            :class="['tab', selectTabIndex === index ? 'tab-select' : '']"
            @click="selectTab(index)"
          >{{ item }}</span>
          <div class="tab-underline" :style="{ left: tabUnderLeft + 'px' }"></div>
        </div>
      </div>
      <div class="thing-content-view">
        <div class="flex-view">
          <div class="main-content" v-show="selectTabIndex === 0">
            <div class="text" v-html="detailData.content"></div>
          </div>
          <div class="thing-comment" v-show="selectTabIndex === 1">
            <div class="publish flex-view">
              <img :src="AvatarIcon" class="mine-img">
              <textarea 
                placeholder="来分享一下你的想法吧" 
                class="content-input" 
                ref="commentRef"
              ></textarea>
            ...
            <div class="tab-view flex-view">
              <div class="count-text">评论 ({{ commentData.length }})</div>
              ...
              <div class="comment-item" v-for="item in commentData" :key="item.id">
                <div class="flex-item flex-view">
                  ...
                  <div class="float-right">
                    <span @click="like(item.id)">赞 <span class="num">{{ item.likes }}</span></span>
                  </div>
                </div>
          ...
          <div class="recommend">
            ...
            <div class="things">
              <div class="thing-item" v-for="item in recommendData" :key="item.id" @click="handleDetail(item)">
                <div class="img-view">
                  <img :src="item.cover" alt="推荐商品">
                </div>
                <div class="info-view">
                  <div class="thing-name">{{ item.title }}</div>
      ...
      <div class="buy-way">
        <button class="buy-btn" @click="handleOrder(detailData)">立即购买</button>
      </div>
    </div>
    <Footer/>
  </div>
...
const getThingDetail =()=> {
  detailApi({id: thingId.value}).then(res => {
    ...
    detailData.value.cover = 'http://localhost:9100/api/upload/image/' + detailData.value.cover
  }).catch(err => {
